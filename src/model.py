...
from transformers.models.led.modeling_led import LEDDecoder, LEDDecoderAttention, ACT2FN, LEDEncoder , LEDSeq2SeqLMOutput, LEDSeq2SeqModelOutput, LEDEncoderBaseModelOutput, LEDLearnedPositionalEmbedding
...
class GraphEncoder():
    def __init__(self, config):
        # TODO: put all the correct split
        self.graphs = {
            "train": pickle.load(open(f"{config['graph_data_path']}/val_graphs_with_features.pkl", 'rb')),
            "val": pickle.load(open(f"{config['graph_data_path']}/val_graphs_with_features.pkl", 'rb')),
            "test": pickle.load(open(f"{config['graph_data_path']}/val_graphs_with_features.pkl", 'rb'))
        }

        self.GM = GATModel(config.GAT_dim, config.GAT_embedding_size,
                           heads=[config['GAT_heads'], config['GAT_heads'], config['GAT_heads']])
...
class LEDKDecoder(LEDPreTrainedModel):
    ...
    def __init__(self, config: LEDConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.attention_mask = AttentionMaskConverter(True)
    ...
    def forward(
        ...
        if input_shape[-1] > 1:
            combined_attention_mask = self.attention_mask._make_causal_mask(
                input_shape, inputs_embeds.dtype, past_key_values_length=past_key_values_length
            ).to(self.device)

        if attention_mask is not None and combined_attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            combined_attention_mask = combined_attention_mask + self.attention_mask._expand_mask(
        ...
        if encoder_hidden_states is not None and encoder_attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
            encoder_attention_mask = self.attention_mask._expand_mask(encoder_attention_mask, inputs_embeds.dtype, tgt_len=input_shape[-1])
...
class LEDKModel(LEDPreTrainedModel):
    ...
    def forward(
        ...
        if graph_encoder_outputs is not None and self.is_merge_encoders:
            ...
            hidden_states = nn.functional.dropout(hidden_states, p=self.graph_activation_dropout,
